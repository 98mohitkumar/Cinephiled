      ...
      <LayoutContainer className='relative z-20'>
        ...
        <div className='relative z-20'>
          {renderList?.length > 0 ? (
            <Fragment>
              <div className='my-5 pb-6 text-center lg:my-10'>
                <h1 className='text-zinc-400 w-full text-[calc(1.375rem_+_1.5vw)] font-semibold xl:text-[2.25rem]'>
                  TV Shows available on <br className='hidden max-sm:block' />
            ...
            <PlaceholderText height='large'>No TV Shows available on {providerName} in this region</PlaceholderText>
          )}
        </div>
      </LayoutContainer>
    </Fragment>
...
export const getServerSideProps = async (ctx) => {
  try {
    ...
    const [providerMoviesData, regionsData] = await Promise.all([providerTvRes.json(), regionsRes.json()]);
