          ...
          <CastGrid>
            {cast.map((item) => (
              <CastWrapper key={item.id}>
                ...
                <div className='mt-3'>
                  <Span className='font-bold movieCastHead block'>{item?.roles[0]?.character}</Span>
...
Cast.getInitialProps = async (ctx) => {
  try {
    ...
    const releaseYear = data?.first_air_date ? new Date(data?.first_air_date).getFullYear() : "TBA";
